...
export async function onRequestPost({ env, data, request }) {
  ...
  const u = await env.DB.prepare(`
    SELECT id,password_hash,password_salt,password_iter
  ...
  // bump session_version => force re-login on other devices
  await env.DB.prepare(`
    UPDATE users
    SET password_hash=?, password_salt=?, password_iter=?, password_algo='pbkdf2_sha256',
