import { json, readJson, normEmail, hasRole, getRolesForUser, pbkdf2Hash, randomB64, sha256Base64, audit } from "../../_lib.js";
...
export async function onRequestGet({ env, data, request }){
  ...
  const r = await env.DB.prepare(`
    SELECT id,email_norm,display_name,status,created_at,updated_at,last_login_at,last_ip_hash
    FROM users
    WHERE ${where}
...
export async function onRequestPost({ env, data, request }){
  ...
  if (!["admin","staff"].includes(role) && !sess.roles.includes("super_admin")) return json(403,"forbidden",{message:"only super_admin can create non admin/staff"});
  if (!["super_admin","admin","staff"].includes(role)) return json(400,"invalid_input",{message:"role invalid"});
  ...
  const rid = await ensureRole(env, role);
...
export async function onRequestPut({ env, data, request }){
  ...
  // admin tidak boleh mengubah super_admin
