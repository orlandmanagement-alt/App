import { json, readJson, sha256Base64, randomB64, pbkdf2Hash, normEmail, ensureRole } from "../../_lib.js";
...
export async function onRequestPost({ request, env }) {
  ...
  const now = nowSec();
  const role_id = await ensureRole(env, "super_admin");
  ...
  await env.DB.prepare(
    `INSERT INTO users (
      ...
      password_hash,password_salt,password_iter,password_algo,session_version
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,1)`
  ).bind(
