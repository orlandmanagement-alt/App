// App/functions/api/setup/bootstrap.js
// POST /api/setup/bootstrap
// Create first Super Admin WITHOUT login.
// Only allowed when no active super_admin exists.
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (exists) {
      return json(409, "conflict", { message: "super_admin already exists" });
    }
    ...
    if (used) {
      return json(409, "conflict", { message: "email already used" });
    }

    const now = Math.floor(Date.now() / 1000);

    // Ensure role exists
    let role = await env.DB.prepare(
    ...
    // Note: your schema requires email_hash NOT NULL UNIQUE
    ...
    // Insert user + role mapping in a transaction-like sequence
    await env.DB.prepare(
      `INSERT INTO users (
    ...
    // Return JSON (no HTML), so your frontend can render debug properly
    return json(200, "ok", { created: true, user_id, email_norm: email });
  } catch (e) {
    // Always return JSON instead of Cloudflare HTML error page
    return json(500, "server_error", {
      message: String(e?.message || e),
      // helpful hints:
      hint: "Check D1 schema (users/roles/user_roles) and Pages bindings/env vars.",
    });
  }
}
