...
export async function onRequestGet({ env, data, request }) {
  ...
  const binds = [];
  let where = "1=1";
  if (q) { where += " AND (type LIKE ? OR summary LIKE ?)"; binds.push(`%${q}%`,`%${q}%`); }
  ...
  const r = await env.DB.prepare(`
    SELECT id,severity,type,status,summary,owner_user_id,created_at,updated_at
    FROM incidents
    WHERE ${where}
  ...
  const next_cursor = has_more
    ? b64e(JSON.stringify({ created_at: items[items.length-1].created_at, id: items[items.length-1].id }))
    : null;

  return json(200,"ok",{ incidents: items, next_cursor });
}
