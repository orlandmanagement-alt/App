import { json, readJson, sha256Base64, randomB64, pbkdf2Hash, hasRole, getRolesForUser } from "../_lib.js";

export async function onRequestGet({ env }) {
  // sederhana: list 100 user terakhir (order by updated_at)
...
export async function onRequestPost({ request, env, data }) {
  ...
  // admin tidak boleh create super_admin (opsional rule)
  if (roleName === "super_admin" && !hasRole(sess.roles, ["super_admin"])) return json(403, "forbidden", { message:"only super_admin can create super_admin" });
  ...
  await env.DB.prepare("INSERT INTO user_roles (user_id,role_id,created_at) VALUES (?,?,?)").bind(user_id, role.id, now).run();
...
export async function onRequestPut({ request, env, data }) {
  ...
  if (action === "reset_request") {
    // placeholder: nanti bisa dikembangkan jadi self-service email
    const email = String(body?.email || "").trim().toLowerCase();
