import { json, readJson, sha256Base64, randomB64, pbkdf2Hash } from "../_lib.js";
...
export async function onRequestPost({ request, env }) {
  ...
  const exists = await env.DB.prepare(
    `SELECT 1 AS ok
     ...
     WHERE r.name='super_admin' AND u.status='active' LIMIT 1`
  ).first();
  ...
  if (!role) {
    ...
    await env.DB.prepare("INSERT INTO roles (id,name,created_at) VALUES (?,?,?)").bind(rid, "super_admin", now).run();
    role = { id: rid };
  }
