import { json, readJson, pbkdf2Hash, randomB64, sha256Base64, normEmail, ensureRole, audit } from "../../_lib.js";
...
export async function onRequestPost({ env, request }){
  ...
  if(!token || !email.includes("@") || password.length < 10) return json(400,"invalid_input",{message:"token/email/password invalid"});
  ...
  const now=nowSec();
  const role_id = await ensureRole(env, String(inv.role||"staff"));
  ...
  await env.DB.prepare("UPDATE invites SET used_at=?, used_by_user_id=? WHERE id=?").bind(now, user_id, invite_id).run();
